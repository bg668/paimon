...
本文档面向希望接入和调用 `paimonsdk` 的开发者，重点说明各项能力的调用方式、关键参数含义、运行时行为，以及最小可用示例。全文以仓库中的 `src/paimonsdk/` 目录实现和 `tests/` 中的测试行为为准，不以早期设计稿或未来规划为准。

本文档与《paimonsdk 功能说明》的定位不同：

- 《paimonsdk 功能说明》回答“已经实现了什么能力、边界在哪里”。
- 《paimonsdk 开发者参考》回答“这些能力应该怎么调用、调用后会发生什么、如何快速接入”。

从定位上看，`paimonsdk` 更接近可嵌入应用中的 Agent Runtime Kernel，而不是完整的 Agent 平台。它覆盖了多轮对话、工具调用、事件分发、流式处理、运行状态和模型适配等核心能力；技能系统、长期记忆、任务调度、业务工作流和权限体系仍应由上层应用实现。

文档中的示例统一使用 `paimonsdk` 作为导入名，因为本仓库的正式包名即为 `paimonsdk`。如果你的外部工程对该包做过二次封装或发布了兼容别名，请按实际导出名调整导入语句。
...
仓库中已经有一个更完整的终端示例 [examples/chat_demo.py](/Users/biguncle/project/paimon/examples/chat_demo.py)。本章给出的代码会更短，目的是先建立最小接入闭环，后续复杂用法再分别在后面章节展开。
...
from paimonsdk import Agent, AgentOptions, AssistantMessage, ModelInfo, TextContent
...
async def main() -> None:
    ...
    adapter = OpenAIAdapter(
        client,
        request_config=OpenAIRequestConfig(temperature=0.2),
    )
    ...
    await agent.prompt("请用两句话介绍 paimonsdk 的定位。")
...
- `Agent` 本身不直接连接模型，真正负责与模型通信的是 `stream_fn`，这里使用的是 `OpenAIAdapter.stream_message`。
- `OpenAIAdapter` 会根据 `ModelInfo.api` 自动分发到底层协议实现；当前支持 `chat.completions` 和 `responses`。
...
- `system_prompt` 会保存在 `AgentContext.system_prompt` 中；默认 OpenAI 适配器会按所选协议自动注入：`chat.completions` 下映射为首条 system message，`responses` 下映射为 `instructions`。
...
3. 运行时调用 `stream_fn`。在这个示例里，`stream_fn` 实际上会转发到 `OpenAIAdapter.stream_message()`。
4. `OpenAIAdapter` 根据 `ModelInfo.api` 选择具体实现，再把内部消息结构转换成对应协议所需的请求格式，并向模型发起流式请求。
...
从使用习惯上看，`AgentOptions` 可以分成五组：

| 配置组 | 代表字段 | 作用 |
| --- | --- | --- |
| 基础配置 | `system_prompt`、`model`、`messages` | 定义初始上下文和模型身份 |
| 模型接入 | `stream_fn` | 声明 kernel 如何调用底层模型 |
...
- 如需 provider 请求参数，则在 adapter 自己的配置对象中设置

其中最重要、也最容易被忽略的是 `stream_fn`。`Agent` 本身不会自行决定如何连接模型；如果没有配置 `stream_fn`，运行时会在实际执行时报错。因此，只要你打算真正跑通一次调用，就必须提供一个可用的 `stream_fn`，最常见的做法就是使用 `OpenAIAdapter.stream_message`。像 `api_key`、`temperature`、`top_p`、`max_tokens` 这类 provider 请求参数，不再由 `AgentOptions` 承载，而是交给 adapter 自己的配置。
...
| `api` | 所用协议类型，当前支持 `chat.completions` 或 `responses` |
| `base_url` | 可选的兼容接口地址 |
...
- `api="chat.completions"`：使用 OpenAI Chat Completions 协议
- `api="responses"`：使用 OpenAI Responses 协议

如果你要接兼容 OpenAI 协议的网关，仍然建议沿用 `provider="openai"`，再根据网关实际支持的协议选择 `api`，并把地址差异放在 `base_url` 上。
...
当前 SDK 已把会话序列化能力做成公开 API。除了运行中的 `Agent` 之外，你还需要知道四个对象：

- `AgentSession`：稳定会话快照，保存 `system_prompt`、`model`、`thinking_level`、transcript、工具引用、队列快照、`metadata`、稳定边界和事件日志。
- `Checkpoint`：带 `checkpoint_id` 和 `created_at` 的恢复载体，内部持有一份 `AgentSession` 与对应稳定边界。
- `StableBoundary`：声明当前导出点是否可恢复。当前只支持 `initialized` 和 `idle`，不支持 token 级恢复。
- `RecordedEvent`：持久化后的事件日志项，保存 `event_id`、`run_id`、`turn_id`、`seq`、`timestamp`、`turn_index` 和序列化 payload。

接入时有三个关键认知：

- `Agent.export_session()` / `Agent.export_checkpoint()` 只能在稳定边界调用，否则会抛出 `SessionExportError`。
- `Agent.from_session()` / `Agent.from_checkpoint()` 会把 Agent 恢复到一个可继续运行的稳定状态。
- session 中保存的是 `ToolReference`，而不是可执行工具对象本身；恢复后仍需要通过 `AgentOptions.tools` 提供实际工具实现。
...
```python
from paimonsdk import TextContent, UserMessage
...
### 4.10 session 导出、checkpoint 恢复与 replay

如果你需要把单 agent 会话跨进程、跨请求或跨线程保存下来，当前 SDK 提供的是稳定边界导出与恢复，而不是运行中断点续传。

最常用的 API 是：

- `agent.export_session()`
- `agent.export_checkpoint()`
- `Agent.from_session(session, options)`
- `Agent.from_checkpoint(checkpoint, options)`
- `agent.replay_run(run_id=None)`
...
last_run_events = restored.replay_run()
```

这里有几个边界必须明确：

- 导出只能发生在稳定边界。若当前仍在流式中、还有 pending tool calls，或 run 尚未收尾，会抛出 `SessionExportError`。
- `Checkpoint` 导入时会校验 `schema_version`、`stable_boundary` 和事件日志一致性；不一致会抛出 `CheckpointImportError`。
- `AgentOptions.session` 与 `Agent.from_session()` / `Agent.from_checkpoint()` 解决的是同一类问题：从稳定会话进入运行态。前者适合在构造时直接注入，后者更适合从外部持久化载体恢复。
...
| `session` | `None` | 直接从已有 `AgentSession` 恢复稳定会话 |
| `tools` | `[]` | 作为当前会话可用工具列表 |
...
- `session` 适合在你已经拿到 `AgentSession` / `Checkpoint.session` 时直接恢复稳定会话。
...
- 你还没有完整 session，只想基于消息历史恢复最小上下文

什么时候优先使用 `session` 而不是 `messages`：

- 你需要同时恢复 transcript、steering / follow-up 队列、稳定边界和事件日志
- 你需要继续使用 `replay_run()`、`export_checkpoint()` 等会话级能力
- 你需要保留 `session_id`、`thinking_level` 和已有 `metadata`
- 你在测试或调试 continuation、队列和工具语义

如果 `AgentOptions.session` 非空，当前实现会优先按 session 恢复 `system_prompt`、`model`、`messages`、队列模式和 `thinking_level`；`options.metadata` 会在此基础上做合并，`options.tools` 仍用于提供运行期可执行工具。
...
`temperature`、`top_p`、`max_tokens`、`api_key` 这类 provider 请求参数，已经从 `AgentOptions` 移到 adapter 自己的配置对象。对当前仓库的 OpenAI 适配器，推荐通过 `OpenAIRequestConfig` 设置。
...
这样做的原因是：这些字段并不是 runtime kernel 的稳定语义，而是某个具体 provider 的请求参数。把它们放回 adapter，能避免 `AgentOptions` 演化成 provider 选项的大杂烩。
...
`base_url`、`api_key` 和生成参数的推荐接法，是在创建外部客户端和 adapter 时配置，而不是塞进 `AgentOptions`。例如兼容 OpenAI 的代理地址，仍然应该在 `AsyncOpenAI(base_url=...)` 时确定。
...
其中前两个字段已经在当前队列实现中实际生效，`thinking_level` 是当前会话的 kernel 状态字段。
...
- 对于 `metadata`，把它视为会话级附加信息更准确；是否传给具体 provider，应由 adapter 自己决定。
- `session_id` 已移动到 `AgentSession` / `Checkpoint` 协议，不再由 `AgentOptions` 直接声明。
- `on_payload`、`thinking_budgets`、`max_retry_delay_ms` 已不再属于 kernel 公开配置。
...
| `artifacts` | 最小 artifact 引用集合，元素类型为 `ToolArtifactRef` |
| `error` | 结构化错误对象，类型为 `ToolError \| None` |
| `status` | 显式结果状态，区分 `ok`、`error`、`blocked` |
...
- 被前置 Hook 阻断时，`status` 会是 `BLOCKED`，同时仍会产生一条稳定的 `ToolResultMessage`。
...
仓库里的 [examples/chat_demo.py](/Users/biguncle/project/paimon/examples/chat_demo.py) 就采用了这个模式：用一个计数器记住已输出长度，只打印新增文本。
...
```python
from paimonsdk import ImageContent
...
- 当前 tool execution 事件还会带 `assistant_message_id`，便于把工具执行过程挂回对应 assistant 消息
...
### 7.6 事件 envelope 与关联字段

当前事件协议已经稳定到可以作为外部接口使用。所有事件都会带统一 envelope 字段：
...
- `MessageStartEvent` / `MessageUpdateEvent` / `MessageEndEvent` / `TurnEndEvent` 会带 `message_id`
- `ToolExecutionStartEvent` / `ToolExecutionUpdateEvent` / `ToolExecutionEndEvent` 会带 `assistant_message_id`
- 持久化后的 `RecordedEvent` 还会额外保存 `turn_index`

如果你在做日志落盘、外部状态同步或 replay，不要自己再推断顺序或消息归属，直接使用这些字段。

### 7.7 流式输出最小示例

下面这个例子展示了如何通过事件监听实现一个最小的终端流式输出。它和仓库中的 [examples/chat_demo.py](/Users/biguncle/project/paimon/examples/chat_demo.py) 是同一种思路，但更短一些。

```python
import asyncio

from paimonsdk import Agent, AgentOptions, AssistantMessage, TextContent
...
from paimonsdk import AgentToolResult, TextContent
...
| `artifacts` | 最小 artifact 引用集合，供上层系统关联真实产物 |
| `error` | 结构化错误对象，供运行时和应用层统一消费 |
| `status` | 显式结果状态，当前支持 `OK`、`ERROR`、`BLOCKED` |
| `metadata` | 供工具内部或 Hook 透传的附加信息 |
...
- `content` 是“给模型和用户看的”
- `details` 是“给程序看的”
- `artifacts` 是“给外部产物治理和跳转看的”
- `error` / `status` 是“给错误语义和控制流看的”
...
```python
from paimonsdk import BeforeToolCallResult
...
```python
from paimonsdk import AfterToolCallResult, TextContent
...
- `OpenAIAdapter`：统一门面，推荐默认使用；按 `ModelInfo.api` 自动选择具体协议实现
- `OpenAIChatCompletionsAdapter`：只处理 `chat.completions`
- `OpenAIResponsesAdapter`：只处理 `responses`

大多数场景下，推荐直接使用 `OpenAIAdapter`：

```python
from openai import AsyncOpenAI

from paimonsdk import Agent, AgentOptions, ModelInfo
from paimonsdk.adapters import OpenAIAdapter, OpenAIRequestConfig
...
如果你明确只想绑定某一种协议，也可以直接实例化具体类；但需要保证类名与 `model.api` 一致，否则调用会返回错误消息而不是静默降级。

当前默认 OpenAI 适配链路做了几件事：
...
5. 由 adapter 自己管理 provider 请求参数
...
`OpenAIAdapterError` 是 OpenAI 适配层内部使用的错误类型，用于表示消息映射、协议分发或响应解析出现了适配器级问题。

当前源码中，典型触发点包括：

- 遇到不支持的消息角色
- `model.api` 不受支持
- OpenAI 响应里缺少当前协议要求的关键字段
...
7. 以为 provider 地址和请求参数应该放在 `AgentOptions`

对于当前默认 OpenAI 适配器，更稳妥的做法是：在创建 `AsyncOpenAI` 客户端时传入 `base_url`，在 `OpenAIRequestConfig` 中设置 `api_key`、`temperature`、`max_tokens` 等请求参数。
...
2. 创建 `OpenAIAdapter`，并在 adapter 配置里设置 provider 请求参数
3. 构造 `AgentOptions(system_prompt, model, stream_fn)`
4. 注册一个轻量事件监听器，处理 `message_start` / `message_update` / `message_end`
5. 在循环中调用 `await agent.prompt(user_input)`

仓库中的 [examples/chat_demo.py](/Users/biguncle/project/paimon/examples/chat_demo.py) 就是这一模式的完整参考。
...
- `metadata`
- 如需稳定会话标识，则读取 `AgentSession.session_id`
...
| `AgentSession` | 稳定会话快照 | 会话导出、恢复 |
| `Checkpoint` | 可恢复的稳定边界封装 | checkpoint 导出、恢复 |
| `StableBoundary` | 稳定边界描述 | session / checkpoint 校验 |
| `RecordedEvent` | 持久化事件日志项 | replay、日志落盘 |
| `EventEnvelope` | 事件公共 envelope 字段 | 事件协议、外部同步 |
| `ToolArtifactRef` | 工具产物引用 | artifact 跳转、审计 |
| `ToolError` | 结构化工具错误 | 错误治理、UI 呈现 |
...
| `OpenAIAdapter` | 默认 OpenAI 统一入口 | 模型接入 |
| `OpenAIChatCompletionsAdapter` | OpenAI Chat Completions 专用适配器 | 显式绑定 `chat.completions` |
| `OpenAIResponsesAdapter` | OpenAI Responses 专用适配器 | 显式绑定 `responses` |
...
所有事件当前都带稳定 envelope 字段：`event_id`、`run_id`、`turn_id`、`seq`、`timestamp`。如果需要把消息和工具执行关联起来，再额外读取 `message_id` 和 `assistant_message_id`。
...
| `session` | `AgentSession \| None` | `None` | 已生效 | 从稳定 session 恢复 |
...
- `model`
- `stream_fn`
- 如需 provider 请求参数，在 adapter 配置里声明
- 如需工具则加 `tools`

### 13.4 示例索引

### 13.5 `AgentLoopConfig` 职责边界

`AgentLoopConfig` 不再是 `AgentOptions` 的机械透传副本。它只承载单轮运行真正需要的内核字段：
...
像 `api_key`、`temperature`、`top_p`、`max_tokens` 这类 provider 请求参数，不再进入 `AgentLoopConfig`。如果你的自定义 `stream_fn` 需要这些信息，推荐做法是像默认 OpenAI 适配器一样，把它们绑定到 adapter 自己的配置对象里。
...
如果你之前通过 `AgentOptions.session_id` 传会话标识，现在应把它放进 `AgentSession.session_id`，或在恢复时直接通过 `Agent.from_session()` / `Agent.from_checkpoint()` 进入运行态。
