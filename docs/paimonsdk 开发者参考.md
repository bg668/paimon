...
仓库中已经有一个更完整的终端示例 [examples/chat_demo.py](/Users/biguncle/project/uu-work/examples/chat_demo.py)。本章给出的代码会更短，目的是先建立最小接入闭环，后续复杂用法再分别在后面章节展开。
...
其中最重要、也最容易被忽略的是 `stream_fn`。`Agent` 本身不会自行决定如何连接模型；如果没有配置 `stream_fn`，运行时会在实际执行时报错。因此，只要你打算真正跑通一次调用，就必须提供一个可用的 `stream_fn`，最常见的做法就是使用 `OpenAIChatCompletionsAdapter.stream_message`。像 `api_key`、`temperature`、`top_p`、`max_tokens` 这类 provider 请求参数，不再由 `AgentOptions` 承载，而是交给 adapter 自己的配置。
...
仓库里的 [examples/chat_demo.py](/Users/biguncle/project/uu-work/examples/chat_demo.py) 就采用了这个模式：用一个计数器记住已输出长度，只打印新增文本。
...
下面这个例子展示了如何通过事件监听实现一个最小的终端流式输出。它和仓库中的 [examples/chat_demo.py](/Users/biguncle/project/uu-work/examples/chat_demo.py) 是同一种思路，但更短一些。
...
当前仓库已经提供了一个开箱可用的适配器：[openai_chatcompletions.py](/Users/biguncle/project/uu-work/src/paimonsdk/adapters/openai_chatcompletions.py) 中的 `OpenAIChatCompletionsAdapter`。
...
2. 创建 `OpenAIChatCompletionsAdapter`，并在 adapter 配置里设置 provider 请求参数
...
仓库中的 [examples/chat_demo.py](/Users/biguncle/project/uu-work/examples/chat_demo.py) 就是这一模式的完整参考。
