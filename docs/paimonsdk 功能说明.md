...
本文档用于说明 `paimonsdk` 已实现的功能与特性，帮助调用者从接入和使用的角度理解 SDK 的能力边界、运行方式和扩展点。全文以仓库中的 `src/paimonsdk/` 目录实现为依据，结论以代码和测试行为为准，不以设计稿、计划文档或预期目标为准。
...
`paimonsdk` 当前实现的是一个偏运行时内核的 Python Agent SDK。它不以提供完整的平台型能力为目标，也不直接承载技能系统、任务调度、长期记忆、业务策略编排或应用侧上下文构造；它主要负责统一一轮 Agent 运行中最关键的基础能力，包括消息模型、主循环、工具执行、事件分发、状态归约和模型适配。
...
因此，调用者应将 `paimonsdk` 理解为“可被外部应用嵌入的 Agent Runtime Kernel”，而不是“开箱即用的一体化 Agent 平台”。
...
| --- | --- | --- | --- |
| 对外入口 | `Agent`、消息模型、事件模型、配置模型、错误类型导出 | `src/paimonsdk/__init__.py`、`src/paimonsdk/runtime/__init__.py` | 统一入口，便于外部应用直接接入 |
| 对话运行 | `prompt()`、`continue_()`、`steer()`、`follow_up()`、`abort()`、`wait_for_idle()`、`reset()` | `src/paimonsdk/runtime/agent.py` | 支撑多轮对话和运行中控制 |
| 主循环 | prompt 场景与 continuation 场景下的循环推进 | `src/paimonsdk/runtime/loop.py` | 统一处理模型回复、工具调用和追加消息 |
| 消息模型 | user / assistant / toolResult 消息，以及 text / image / thinking / toolCall 内容块 | `src/paimonsdk/runtime/models.py` | 统一对话、工具结果和流式中间态的数据结构 |
| 工具调用 | 工具查找、参数预处理、参数校验、结构化结果 / 错误 / artifact 引用 | `src/paimonsdk/runtime/tool_executor.py` | 降低工具接入复杂度，并让工具结果更易治理 |
| 工具扩展 | `before_tool_call`、`after_tool_call`、`on_update` | `src/paimonsdk/runtime/config.py`、`src/paimonsdk/runtime/tool_executor.py` | 允许业务侧插入策略、治理和进度上报 |
| 执行模式 | 串行和并行两种工具执行模式 | `src/paimonsdk/runtime/tool_executor.py` | 在一致性与效率之间做平衡 |
| 流式处理 | 流式响应统一收敛为最终消息，并持续产出更新事件 | `src/paimonsdk/runtime/stream_handler.py` | 支持 CLI / UI 的增量呈现 |
| 事件机制 | agent、turn、message、tool execution 四类事件 | `src/paimonsdk/runtime/events.py` | 便于做观测、日志和状态同步 |
| 会话持久化 | `AgentSession`、`Checkpoint`、稳定边界、事件回放 | `src/paimonsdk/runtime/session.py`、`src/paimonsdk/runtime/agent.py` | 支持单 agent 会话导出、恢复和回放 |
| 状态管理 | transcript、当前流式消息、待完成工具调用、错误信息 | `src/paimonsdk/runtime/state.py` | 允许外部系统随时读取运行快照 |
| 运行控制 | 取消令牌与 run 句柄 | `src/paimonsdk/runtime/run_control.py` | 支持中止执行和等待完成 |
| 模型适配 | OpenAI `chat.completions` / `responses` 适配 | `src/paimonsdk/adapters/openai_adapter.py`、`src/paimonsdk/adapters/openai_chatcompletions.py`、`src/paimonsdk/adapters/openai_responses.py` | 提供统一入口与协议专用入口 |
...
当前 SDK 已把“可序列化 session”补成了公开能力。调用者可以围绕 `AgentSession`、`Checkpoint`、`StableBoundary` 和 `RecordedEvent` 来持久化一个单 agent 的稳定运行边界。

其中：

- `AgentSession` 保存会话级稳定信息，包括 `system_prompt`、`model`、`thinking_level`、transcript、工具引用、steering / follow-up 队列、`metadata`、稳定边界和事件日志。
- `Checkpoint` 是对某个稳定边界的封装，额外带有 `checkpoint_id`、创建时间和边界快照，适合作为恢复入口。
- `StableBoundary` 用于声明当前导出点是否可恢复。当前只支持 `initialized` 和 `idle` 这两类稳定边界，不支持 token 级恢复。
- `RecordedEvent` 是稳定事件日志记录，保存事件 envelope 和序列化 payload，可用于回放最近一轮 run。

这里保存的工具信息是 `ToolReference`，目的是保留工具协议和会话可见能力边界，而不是把 Python 可执行对象本身直接序列化。
...
### 10.5 结构化结果、错误与 artifact 引用

当前 SDK 的工具结果契约已经不再只有 `content` 和 `details` 两个字段。工具返回的 `AgentToolResult` 以及最终落地的 `ToolResultMessage`，还支持：

- `artifacts`：最小 artifact 引用集合，每个元素是 `ToolArtifactRef`，包含 `artifact_id`、`kind`、`uri`、`name`、`mime_type` 和 `metadata`
- `error`：结构化 `ToolError`，包含 `code`、`message`、`retryable` 和 `details`
- `status`：显式区分 `ok`、`error`、`blocked`
- `is_error`：供 transcript 和调用侧快速判断是否为错误态

这意味着当前 SDK 已能把“给模型看的内容”和“给应用治理的结构化结果”分开表达：`content` 仍面向模型与对话层，`details` / `artifacts` / `error` / `status` 更面向应用层和 Hook。
...
### 11.3 事件 envelope 与关联字段

当前事件已经不只是运行时内部通知，而是带稳定 envelope 的外部协议。所有事件都会补齐以下基础字段：
...
此外，消息和工具相关事件还带有额外的关联字段：

- message 事件和 `turn_end` 带 `message_id`
- tool execution 事件带 `assistant_message_id`

这些字段使外部应用可以稳定地区分“这是哪一轮 run、哪一轮 turn、哪一条 assistant 消息、哪一次工具调用”，也让事件日志具备回放价值。
...
### 12.4 session 导出、恢复与 replay 边界

除了读取运行中快照，当前 SDK 还支持在稳定边界导出 `AgentSession` 和 `Checkpoint`。调用者可以：

- 通过 `export_session()` 导出完整会话结构
- 通过 `export_checkpoint()` 导出可恢复的 checkpoint
- 通过 `Agent.from_session()` 或 `Agent.from_checkpoint()` 恢复 agent
- 通过 `replay_run()` 基于事件日志回放最近一轮或指定 `run_id` 的事件

这里的边界是明确的：只有 agent 已回到稳定边界、没有流式中间态、没有 pending tool execution 时，导出才会成功；否则会抛出 `SessionExportError`。这符合当前 SDK “只支持稳定边界恢复，不支持 token 级恢复”的定位。
...
当前代码中的 OpenAI 模型适配层已经拆成三个公开入口：

- `OpenAIAdapter`：统一门面，按 `ModelInfo.api` 自动分发
- `OpenAIChatCompletionsAdapter`：只处理 `chat.completions`
- `OpenAIResponsesAdapter`：只处理 `responses`

其中，`chat.completions` 与 `responses` 两条链路都已具备基础消息映射、工具映射、流式事件处理和工具参数增量恢复能力。

## 14. OpenAI 适配说明

### 14.1 消息映射

当前默认 OpenAI 适配链路支持两种协议：

- `chat.completions`：把内部 transcript 映射成 `messages`
- `responses`：把内部 transcript 映射成 `input` / `function_call_output`

两条链路都支持把用户消息中的文本和图片传给模型；都支持把 assistant 消息中的文本和 tool calls 回灌到下一轮；都支持把工具结果消息回写给模型。

其中，工具结果里的图片当前仍会退化为占位文本；assistant 输出图片也还没有在默认 OpenAI 适配器里形成专门的统一映射。
...
当前适配器能把 `chat.completions` chunk 或 `responses` stream event 转换为 SDK 统一流式事件，包括：
...
| `session` | 从已有 `AgentSession` 恢复稳定会话 |
...
| `metadata` | 保存会话级附加信息，供 session 导出和自定义 `stream_fn` 使用 |
| `thinking_level` | 记录当前会话的思考级别 |

### 15.2 已建模但未完全打通的配置项

本轮收缩后，以下字段不再属于 SDK kernel 公开配置，而是分别下沉到 session 协议或 adapter 专属配置：

| 配置项 | 当前状态 |
| --- | --- |
| `session_id` | 保留在 `AgentSession` / `Checkpoint` 协议中，不再由 `AgentOptions` 直接声明 |
| `api_key` / `temperature` / `top_p` / `max_tokens` | 下沉到 OpenAI adapter 自己的 `request_config` |
| `base_url` | 继续由外部客户端初始化负责，例如创建 `AsyncOpenAI(base_url=...)` |
| `on_payload` / `thinking_budgets` / `max_retry_delay_ms` | 因默认主链路不消费，已从 kernel 配置面移除 |

阅读配置项时，需要先区分三层边界：kernel 配置、session 协议字段、adapter 专属请求配置。

## 16. 限制、当前边界与验证结论

### 16.1 `system_prompt` 在默认 OpenAI 链路中已自动进入请求

当前 `system_prompt` 不仅会保存在状态和配置中，默认 OpenAI 适配器还会按所选协议自动注入请求：

- `chat.completions` 下追加为首条 system message
- `responses` 下映射为 `instructions`

因此，在当前默认链路下，`system_prompt` 已经属于默认生效字段，而不再只是“已建模但未注入请求”的保留配置。

### 16.2 当前只有 OpenAI 协议族适配

从当前代码看，还没有其他模型供应商的适配器，也没有 Anthropic、Gemini 或本地模型接入实现。当前 SDK 的适配层能力应被准确理解为“已实现 OpenAI 协议族下的 `chat.completions` 与 `responses` 适配”，而不是“已具备完整多模型平台能力”。
...
当前 `src/paimonsdk/adapters/` 下的 OpenAI 适配器导入路径已经与 `paimonsdk` 包名保持一致。在当前仓库执行测试时，现有回归用例可以通过。
...
仓库中的 `examples/chat_demo.py` 提供了一个简单的终端聊天示例。它展示了如何创建 OpenAI 客户端、构造 `OpenAIAdapter`、创建 `Agent`、订阅消息事件，并在终端中打印流式文本。示例配置中的 `model.api` 可在 `chat.completions` 和 `responses` 之间切换。这份示例适合作为调用者理解 SDK 接入最短闭环的辅助材料。
...
| `SessionExportError` | 在非稳定边界导出 session / checkpoint |
| `CheckpointImportError` | 导入的 checkpoint / session 与稳定边界约束不一致 |
...
这些事件当前都带统一 envelope 字段：`event_id`、`run_id`、`turn_id`、`seq`、`timestamp`；其中消息相关事件还会附带 `message_id`，工具相关事件会附带 `assistant_message_id`。
...
从当前代码实现来看，`paimonsdk` 已具备一个可工作的 Agent Runtime 内核所需的核心能力：能够统一管理消息、上下文、主循环、工具调用、事件流和状态视图，并通过 OpenAI Chat Completions 适配层与具体模型接口连接起来。
