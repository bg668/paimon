...
| 模型适配 | OpenAI Chat Completions 适配 | `src/paimonsdk/adapters/openai_chatcompletions.py` | 当前可直接接 OpenAI 风格接口 |
...
当前 `src/paimonsdk/adapters/openai_chatcompletions.py` 的导入路径已经与 `paimonsdk` 包名保持一致。在当前仓库执行 `uv run pytest -q` 时，现有 27 项测试可以通过。
...
仓库中的 `examples/chat_demo.py` 提供了一个简单的终端聊天示例。它展示了如何创建 OpenAI 客户端、构造 `OpenAIChatCompletionsAdapter`、创建 `Agent`、订阅消息事件，并在终端中打印流式文本。这份示例适合作为调用者理解 SDK 接入最短闭环的辅助材料。
